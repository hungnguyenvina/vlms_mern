...
exports.updateBasicInfoOfCourse = function(req,res){

    let form = {};
    form = {
        ...req.body
    }
    console.log('/instructor/update_course_info....');
    console.log(form);
    ...
    .exec(function(err,courseResult){
        if(err) return res.status(400).json({success: false,course:null, message:'Cannot update course info '+err});
        return res.status(200).json({success: true, course:courseResult, message:'Update course successfully!'});
    });
}
...
exports.getAllCoursesUserEnrolled = function(req,res){
    ...
    User.findOne(
        ...
        (err,user) => {
            ...
            //console.log(user);
            let enrolledCourses = user.history;
            for( var i = 0; i < enrolledCourses.length; i++){ 
                //console.log('item course');
                //console.log(enrolledCourses[i]);
                courses.push(mongoose.Types.ObjectId(enrolledCourses[i].id));
            }

            console.log('courses of a user');
            console.log(courses);
            ...
            .exec(function (err, results) {
                ...
                Lession.find({})
                    .exec(function (err, lessions) {
                        lessions.forEach(item => {
                            lessionArrays.push(item.course_id+';'+item._id)
                        });

                        let finalArray = [];
                        
                for(a=0;a<newResult.length;a++){
                    let defaultLessionID = "";
                    for(b=0;b<lessionArrays.length;b++){
                        const itemLession = lessionArrays[b].split(';');
                        if(String(itemLession[0])===String(newResult[a].id)) {
                            defaultLessionID=itemLession[1];
                            break;
                        }
                    }
                    ...
                    finalArray.push({
                        ...newResult[a],
                        default_lession_id: defaultLessionID
                    });
                }
